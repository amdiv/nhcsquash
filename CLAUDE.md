...
- **[index.html](index.html)** (~2,000 lines) — self-contained: markup, a `<style>` block (lines 10–1359), and a `<script>` block (lines 1852–2000). No framework, no module system, no separate CSS or JS file, no `package.json`.
...
**Court carousel** (`#carousel`, lines 1853–1879) — six static `.carousel-slide` divs, auto-advancing every 3.5s. The dots are hand-written markup: adding or removing a court photo means editing the slide, its matching `.carousel-dot` with a sequential `data-index`, and the "6 Courts Available" badge text.

**Video carousel** (`#vc-*`, lines 1890–1997) — fetched from the function at load, thumbnails built by string concatenation into `innerHTML`. Notes:
...
- Showcase `12314886` is hardcoded in **two places**: the function's URL and the "View all on Vimeo" link ([index.html:1710](index.html#L1710)). Change both together.

## Editing gotchas

**The promo banner is time-limited.** `#promo-banner` currently runs an August first-anniversary offer. Authored comments at [index.html:36](index.html#L36) and [index.html:1384](index.html#L1384) explain how to disable it (`display:none` or delete the block) and how to swap in a new offer — follow them rather than restyling.

**The phone number is duplicated.** `0419688262` appears in six `tel:` links plus display text in the CTA band and footer. Changing it means all of them.
